...
import { environment, UI_USER_ENVIRONMENT } from '../../core/environment';
import { UI_USER_NAVIGATION } from '../../core/navigation';
import { UI_USER_NATIVE_ENVIRONMENT } from '../../core/tokens';
...
import { UI_USER_APP_TYPE_POLICIES } from '../../core/policies';

@Injectable()
export class UiUserRemoteSetupService implements OnDestroy {
  constructor(
    private _versionService: UiCommonVersionService<UiVersion>,
    private _layoutNavigationService: UiLayoutNavigationService,
    @Inject(UI_USER_NATIVE_ENVIRONMENT) @Optional() private _isNative?: boolean
  ) {}
  ...
  start() {
    ...
    if (!this._isNative && UI_USER_NAVIGATION.topNodes.length > 0) {
      this._layoutNavigationService.registerSecondToolbarRowNavigationNodes(UI_USER_NAVIGATION.topNodes);
    }
  }
...
@NgModule({
  imports: [
    UiGraphQLClientOptionsModule.withConfig('user-remote-app', {
...
export class UiUserRemoteCoreModule {}

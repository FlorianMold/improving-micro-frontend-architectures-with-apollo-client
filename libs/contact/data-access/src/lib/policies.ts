 ...
 */
const cacheRedirectFn = ['Contact', 'Title', 'Salutation', 'Country'].reduce(
...
/**
 * Provides all type-policies for accessing the contact-data inside the cache.
 * This field-policy should be used in every application that uses the contact-data-access-service.
 */
export const UI_CONTACT_DATA_ACCESS_SERVICE_ROOT_QUERY_FIELD_POLICIES: {
 ...
 * Provides
 */
export const UI_CONTACT_DATA_ACCESS_SERVICE_TYPE_POLICIES: UiTypePolicies = {
  Contact: {
    fields: {
      /** Whether the contact-detail is opened inside an element. */
      isOpen: {
        read(existing) {
          return !!existing;
        },

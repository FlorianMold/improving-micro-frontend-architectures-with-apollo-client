...
import { UiComponentPortal } from './portal/portal';
import { UiToastInjector, UiToastRef } from './toast-injector';
...
export interface UiActiveToast<C> {
...
export class UiToastrService {
  ...
  currentlyActive = 0;
  toasts: UiActiveToast<any>[] = [];
  overlayContainer?: UiToastContainerDirective;
   ...
   */
  private _findToast(toastId: number): { index: number; activeToast: UiActiveToast<any> } | null {
  ...
  private _buildNotification(
    ...
    const toastInjector = new UiToastInjector(toastPackage, this._injector);
    const component = new UiComponentPortal(config.toastComponent, toastInjector);
    const portal = overlayRef.attach(component, this.toastrConfig.newestOnTop);
    toastRef.componentInstance = portal.instance;
    const ins: UiActiveToast<any> = {
      toastId: this.index,
      title: title || '',
      message: message || '',
      toastRef,

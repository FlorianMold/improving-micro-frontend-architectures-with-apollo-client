...
import { makeReducedQueryAst } from './augmented-query';
...
const compare = (reducedQueryAst: ASTNode, actualQuery: string) => {
...
describe('makeReducedQueryAst', () => {
  afterEach(() => {
    cache.reset();
  });

  it('removes the fields, that are already in the cache', () => {
    const queryInCache = `
        query {
            thing {
                id
    ...
    const reducedQueryAst = makeReducedQueryAst(cache, gql(requestedQuery)) as ASTNode;
  ...
  it('removes fields if the same variables are used', () => {
    ...
    const reducedQueryAst = makeReducedQueryAst(cache, gql(requestedQuery), variables) as ASTNode;
  ...
  it('keeps fields if different variables are used', () => {
    ...
    const reducedQueryAst = makeReducedQueryAst(cache, gql(requestedQuery), requestedVariables) as ASTNode;
  ...
  it('removes the fields, which are marked with the client directive', () => {
    /** The @client directive is only removed, when the client data already exists inside the cache. */
